...
% 定义.mat文件的路径和文件名
po = 137;
filename1 = ['E:\Hyperspectral_recovery\data\XJF\mat\实验原始数据\20230712\' num2str(po) '-1.raw\'];
...
filename8 = [filename1 num2str(po) '-1.raw_2.bmp'];
imgwe = imread(filename8);
pic = ['E:\Hyperspectral_recovery\data\XJF\RGB\' num2str(po) '\' num2str(po) '-1.bmp'];

dataxls = xlsread('E:\Hyperspectral_recovery\data\XJF\重建.xlsx');
ixls = dataxls(:,3:8);
...
i1 = ixls(1+(po-51)*2,1);i2 = ixls(1+(po-51)*2,2);i3 = ixls(1+(po-51)*2,3);
i4 = ixls(1+(po-51)*2,4);i5 = ixls(1+(po-51)*2,5);i6 = ixls(1+(po-51)*2,6);

img2 = img(i1:m-i2,i3:n-i4,:); 
newSize = [ydim,xdim];  % 新的尺寸 [高度, 宽度]
resized_img = imresize(img2, newSize);
resized_img = double(resized_img);
resized_img = resized_img / 255*1.02; 

% 顺时针旋转5度（imrotate默认是逆时针，所以用-5）
img_rotated = imrotate(resized_img, -6 , 'bilinear', 'loose');
...
% 计算裁剪的起始位置（中心裁剪）
start_row = floor((h - original_height) / 2) + 1;
start_col = floor((w - original_width) / 2) + 1;

% 裁剪图像
img_cropped = img_rotated(start_row:start_row + original_height - 1, ...
                          start_col:start_col + original_width - 1, :);


                      
% 顺时针旋转5度（imrotate默认是逆时针，所以用-5）
img_rotated2 = imrotate(imgwe, 6 , 'bilinear', 'loose');

% 获取旋转后图像的尺寸
[h, w, ~] = size(img_rotated2);
...
% 裁剪图像
img_cropped2 = img_rotated2(start_row:start_row + original_height - 1, ...
                          start_col:start_col + original_width - 1, :);
...
rgb(:,:,3) = rgb(:,:,3)/255*i6/1.1;
rgb(:,:,2) = zeros(ydim,xdim);
rgb(:,:,1) = rgb(:,:,1)*i5;
...
subplot(253)
h1 = imhist(rgb(:,:,1));
ylim([0 500]);  % 设置 Y 轴的范围（例如从 0 到 3000）
...
subplot(255)
h3 = imhist(rgb(:,:,3));
ylim([0 800]);  % 设置 Y 轴的范围（例如从 0 到 3000）
set(gca,'FontSize',12,'Fontname', 'Arial','LineWidth',1.5, 'FontWeight', 'bold');
...
subplot(257)
imshow(img_cropped(:,:,1))
axis image;

subplot(258)
h2 = imhist(img_cropped(:,:,1));
ylim([0 500]);  % 设置 Y 轴的范围（例如从 0 到 3000）
set(gca,'FontSize',12,'Fontname', 'Arial','LineWidth',1.5, 'FontWeight', 'bold');

subplot(2,5,9)
imshow(img_cropped(:,:,3))
axis image;

subplot(2,5,10)
h4 = imhist(img_cropped(:,:,3));
xlim([0 1]);  
ylim([0 800]); 
...
% mean1 = sum((0:255) .* h1);
% mean2 = sum((0:255) .* h2);
% var1 = sum(((0:255) - mean1).^2 .* h1);
% var2 = sum(((0:255) - mean2).^2 .* h2);
% covariance = sum(((0:255) - mean1) .* ((0:255) - mean2) .* h1);
% correlationCoefficient = covariance / (sqrt(var1) .* sqrt(var2));
% % 计算余弦相似度
% cosineSimilarity1 = sum(h1 .* h2) / (sqrt(sum(h1.^2)) * sqrt(sum(h2.^2)));
% 
% % 归一化直方图
% h3 = h3 / sum(h3);
% h4 = h4 / sum(h4);
% mean3 = sum((0:255) .* h3);
% mean4 = sum((0:255) .* h4);
% var3 = sum(((0:255) - mean3).^2 .* h3);
% var4 = sum(((0:255) - mean4).^2 .* h4);
% covariance2 = sum(((0:255) - mean3) .* ((0:255) - mean4) .* h3);
% correlationCoefficient2 = covariance2 / (sqrt(var3) .* sqrt(var4));
% % 计算余弦相似度
% cosineSimilarity2 = sum(h3 .* h4) / (sqrt(sum(h3.^2)) * sqrt(sum(h4.^2)));
